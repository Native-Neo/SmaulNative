...
def test_native_wkv_forward_backward():
    ...
    configure(2);rwkv_x_core._wkv_run_chunk=cpu._native_wkv
    try:
        for B,T,H,N in ((1,1,1,4),(1,9,2,4),(1,65,2,8),(2,17,3,8)):
            ...
            loss_n=native[0].square().mean()+native[1].square().mean();loss_r=ref[0].square().mean()+ref[1].square().mean()
            gn=torch.autograd.grad(loss_n,args);gr=torch.autograd.grad(loss_r,ref_args)
            for a,b in zip(gn,gr):assert torch.allclose(a,b,rtol=2e-4,atol=2e-5),(B,T,H,N,(a-b).abs().max().item())
            print(f'PASS B={B} T={T} H={H} N={N}')
    finally:
        rwkv_x_core._wkv_run_chunk=original

if __name__=='__main__':
    test_native_wkv_forward_backward();print('native WKV tests passed')
