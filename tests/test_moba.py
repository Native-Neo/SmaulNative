...
def dense_ref(att,x):
    B,T,C=x.shape;H,N,cs,k_top=att.n_head,C//att.n_head,att.chunk_size,att.top_k;q,k,v=att.receptance(x),att.key(x),att.value(x);q=q.view(B,T,H,N).transpose(1,2);k=k.view(B,T,H,N).transpose(1,2);v=v.view(B,T,H,N).transpose(1,2);n_chunks=(T+cs-1)//cs;pad=n_chunks*cs-T;kc=F.pad(k,(0,0,0,pad)).view(B,H,n_chunks,cs,N);km=kc.mean(3);mask=torch.zeros(B,H,T,T,dtype=torch.bool)
    for i in range(n_chunks):
        ...
        if i:
            for b in range(B):
                for h in range(H):
                    for c in top[b,h].tolist():mask[b,h,lo:hi,c*cs:(c+1)*cs]=True
        for r in range(lo,hi):mask[:,:,r,lo:r+1]=True
