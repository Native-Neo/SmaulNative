...
std::vector<torch::Tensor> wkv_forward(torch::Tensor state, torch::Tensor w, torch::Tensor k, torch::Tensor v,
  ...
  TORCH_CHECK(w.dtype() == torch::kFloat32 && k.dtype() == torch::kFloat32 && v.dtype() == torch::kFloat32 &&
              kk.dtype() == torch::kFloat32 && a.dtype() == torch::kFloat32 && r.dtype() == torch::kFloat32);
  TORCH_CHECK(state.dim() == 4 && w.dim() == 4 && k.dim() == 4 && v.dim() == 4 && kk.dim() == 4 && a.dim() == 4 &&
              r.dim() == 4);
  ...
  TORCH_CHECK(w.sizes() == k.sizes() && w.sizes() == v.sizes() && w.sizes() == kk.sizes() && w.sizes() == a.sizes() &&
              w.sizes() == r.sizes());
