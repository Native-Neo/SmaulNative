...
def stream_dataset(
    ...
    for dataset_name in names:
        ...
        for rel_path in paths:
            ...
            skip = (
                start_record
                if dataset_name == start_dataset and rel_path == start_file
                else 0
            )
            print(
                f"[STREAM] {dataset_name}/{rel_path}"
                + (f" from row {skip:,}" if skip else ""),
                file=sys.stderr,
            )
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Stream and filter HF datasets with no dataset files on disk"
    )
