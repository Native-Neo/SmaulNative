    ...
    def step(self,closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                p.add_(avg.mul(b1).add(p.grad,alpha=1-b1).sign(),alpha=-lr);avg.lerp_(p.grad,1-b2)
        return loss
...
def set_router_only_training(model,router_only):
    ...
    for p in model.parameters():p.requires_grad_(id(p) in gates if router_only else True);n+=p.numel() if p.requires_grad else 0
    return n
class ResumeState:
...
def save_checkpoint(model,optimizer,resume,output_dir,checkpoint_dir,tokenizer_path,save_dtype="fp32",save_optimizer=True):
    output_dir.mkdir(parents=True,exist_ok=True);checkpoint_dir.mkdir(parents=True,exist_ok=True);model=getattr(model,"_orig_mod",model);model.save_pretrained(output_dir,dtype=save_dtype,include_upstream=False);bundled=output_dir/"tokenizer.json"
...
def _remote_token_stream(name,tokenizer,ctx_len,resume):
    ds=file=None;record=0
    if resume.file_path and "::" in resume.file_path:ds,file=resume.file_path.split("::",1);record=resume.record_index
    ...
    for text,pos in stream_dataset(name,start_dataset=ds,start_file=file,start_record=record,with_position=True):
        buf.extend(tokenizer.encode(text)+[tokenizer.eos_token_id])
...
def build_model(args,tokenizer):
    ...
    cfg=RWKVXConfig(vocab_size=tokenizer_vocab_size(tokenizer),n_embd=args.n_embd,n_layer=args.n_layer,n_moba_layer=args.n_moba_layer,head_size=args.head_size);cfg.ctx_len_hint=args.ctx_len;return RWKVXModel(cfg)
def main():
    ...
    if not tokenizer_path.exists():train_tokenizer(Path(args.dataset_dir),tokenizer_path,args.tokenizer_vocab_size,args.stream_dataset,args.tokenizer_max_records)
    ...
    try:train_pretrain(args,model,optimizer,resume,device,tokenizer,scaler) if args.mode=="pretrain" else train_sft(args,model,optimizer,resume,device,tokenizer,scaler)
