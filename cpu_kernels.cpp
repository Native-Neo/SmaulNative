...
static inline uint8_t rqt_fp8_stochastic_code(float value, uint32_t& rng) {
  ...
  if (magnitude < 0.015625f) {
    ...
    threshold = (uint32_t)std::min(1048575.0f, std::max(0.0f, (scaled - (float)base) * 1048576.0f));
  } else {
    uint32_t bits;
    std::memcpy(&bits, &magnitude, sizeof(bits));
    const int exponent = (int)((bits >> 23) & 0xFF) - 127;
    const int exponent_field = exponent + 7;
    const uint32_t mantissa_bits = bits & 0x7FFFFFu;
    const int mantissa = (int)(mantissa_bits >> 20);
    ...
    threshold = mantissa_bits & 0xFFFFFu;
  }

  if (lo == hi || lo == magnitude) return negative ? (uint8_t)(lower | 0x80) : lower;
  const uint32_t u = rqt_xorshift(rng) & 0xFFFFFu;
  const uint8_t code = u < threshold ? upper : lower;
