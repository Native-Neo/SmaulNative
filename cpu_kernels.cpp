...
void lion_step(torch::Tensor p, torch::Tensor g, torch::Tensor m, double lr, double b1, double b2, double wd) {
  ...
  at::parallel_for(0, n, 1 << 20, [&](int64_t begin, int64_t end) {
    int64_t i = begin;
    const __m256 vb1 = _mm256_set1_ps(f_b1), v1_b1 = _mm256_set1_ps(1.0f - f_b1);
    const __m256 vb2 = _mm256_set1_ps(f_b2), v1_b2 = _mm256_set1_ps(1.0f - f_b2);
    const __m256 vwd = _mm256_set1_ps(f_wd), vlr = _mm256_set1_ps(f_lr);
    ...
    for (; i < end; ++i) {
      ...
      pp[i] *= f_wd; pp[i] -= f_lr * (v > 0.0f ? 1.0f : (v < 0.0f ? -1.0f : 0.0f));
...
static inline uint8_t rqt_code(const uint8_t* packed, int index, int bits) {
  ...
  switch (index & 3) {
    case 0: return p[0] >> 2;
    case 1: return ((p[0] & 3) << 4) | (p[1] >> 4);
    case 2: return ((p[1] & 15) << 2) | (p[2] >> 6);
    default: return p[2] & 63;
  }
}
...
static inline void rqt_set_code(uint8_t* packed, int index, int bits, uint8_t code) {
  if (bits == 4) {
    ...
    if (index & 1) p = (p & 0xF0) | code; else p = (p & 0x0F) | (code << 4);
    return;
  }
  uint8_t* p = packed + (index >> 2) * 3;
  switch (index & 3) {
    case 0: p[0] = (p[0] & 0x03) | (code << 2); break;
    case 1: p[0] = (p[0] & 0xFC) | (code >> 4); p[1] = (p[1] & 0x0F) | ((code & 15) << 4); break;
    case 2: p[1] = (p[1] & 0xF0) | (code >> 2); p[2] = (p[2] & 0x3F) | ((code & 3) << 6); break;
    default: p[2] = (p[2] & 0xC0) | code;
  }
}
