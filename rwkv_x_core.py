...
class RWKVXConfig:
    ...
    def approx_param_count(self) -> int:
        ...
        return (
            2 * V * C
            + (L - self.n_moba_layer) * (tmix + cmix)
            + self.n_moba_layer * (4 * C * C + cmix)
        )


def config_for_target_params(
    target_params: int,
    vocab_size: int = 65530,
    n_embd: int = 832,
    n_moba_layer: int = 5,
    ...
    if n_embd % head_size:
        raise ValueError(
            f"n_embd ({n_embd}) must be divisible by head_size ({head_size})"
        )
    ...
    for n_layer in range(4, 80):
        cfg = RWKVXConfig(
            ...
            n_embd=n_embd,
            n_layer=n_layer,
            n_moba_layer=min(n_moba_layer, n_layer - 1),
            head_size=head_size,
        )
    ...
    for t in range(w_c.shape[1]):
        ...
        state = (
            ...
            + torch.bmm(
                torch.bmm(
                    flat,
                    u.reshape(-1, state.shape[-2], 1),
                ),
                av.reshape(-1, 1, state.shape[-1]),
            ).reshape_as(state)
            + torch.bmm(
...
class RWKV_Tmix_x070(nn.Module):
        ...
        with torch.no_grad():
            ...
            def oi(x, scale):
                if x.ndim == 2:
                    nn.init.orthogonal_(
                        x,
                        gain=max(1.0, math.sqrt(x.shape[0] / x.shape[1])) * scale,
                    )
                return x
            ...
            self.w0 = nn.Parameter(
                torch.tensor(
                    [-7 + 5 * (n / (C - 1)) ** (0.85 + r0 ** 0.5) for n in range(C)]
                ).reshape(1, 1, C)
                + 0.5
            )
    ...
    def forward(self, x, v_first, state=None):
        ...
        prev0 = (
            ...
            else torch.zeros(B, 1, C, dtype=x.dtype, device=x.device)
        )
        ...
        v = (
            v
            if self.layer_id == 0
            else v + (v_first - v) * torch.sigmoid(
        ...
        r_, w_, k_, v_, kk_, a_ = (
            z.view(B, T, H, N) for z in (r, w, k, v, kk, a)
        )
        state = (
            torch.zeros(B, H, N, N, dtype=torch.float32, device=x.device)
            if state is None
            else state.float()
        )
        ys = []
...
class RWKV_CMix_MoE(nn.Module):
    def __init__(self, cfg, layer_id):
        ...
        self.experts = nn.ModuleList(
            [RWKV_CMix_x070(cfg, layer_id) for _ in range(self.num_experts)]
        )
        self.gate = nn.Linear(cfg.n_embd, self.num_experts, bias=False)
        self.top_k < 1 and (_ for _ in ()).throw(
            ValueError("num_experts_per_tok must be >= 1")
        )
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, cache=None, use_cache=False):
        ...
        if cache is not None:
            pk, pv = cache
            past = pk.size(2)
            full_k, full_v = torch.cat((pk, k), 2), torch.cat((pv, v), 2)
            if T != 1:
                raise ValueError("cached MOBA attention expects one-token decode")
            cur_start = (past // cs) * cs
            cur_k, cur_v = full_k[:, :, cur_start:], full_v[:, :, cur_start:]
            prev_k, prev_v = full_k[:, :, :cur_start], full_v[:, :, :cur_start]
            if prev_k.size(2) and kt > 0:
                n_prev = (cur_start + cs - 1) // cs
                pad = n_prev * cs - cur_start
                pkc = F.pad(prev_k, (0, 0, 0, pad)).view(B, H, n_prev, cs, N)
                pvc = F.pad(prev_v, (0, 0, 0, pad)).view(B, H, n_prev, cs, N)
                npick = min(kt, n_prev)
                top = torch.einsum(
                    "bhn,bhcn->bhc", q.mean(2), pkc.mean(3)
                ).topk(npick, -1).indices
                idx = top.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, -1, cs, N)
                sk = torch.gather(pkc, 2, idx).reshape(B, H, npick * cs, N)
                sv = torch.gather(pvc, 2, idx).reshape(B, H, npick * cs, N)
                y = F.scaled_dot_product_attention(
                    q,
                    torch.cat((sk, cur_k), 2),
                    torch.cat((sv, cur_v), 2),
                    is_causal=False,
                )
            else:
                y = F.scaled_dot_product_attention(q, cur_k, cur_v, is_causal=False)
            return self.output(y.transpose(1, 2).reshape(B, 1, C)), (
                full_k,
                full_v,
        ...
        if kt <= 0 or n_chunks <= kt + 1:
            ...
            for i in range(n_chunks):
                ...
                if i == 0:
                    ...
                    npick = min(kt, i)
                    top = torch.einsum(
                        "bhn,bhcn->bhc", qi.mean(2), km[:, :, :i]
                    ).topk(npick, -1).indices
                    idx = top.unsqueeze(-1).unsqueeze(-1).expand(
                        -1, -1, -1, cs, N
                    )
                    sk = torch.gather(kc[:, :, :i], 2, idx).reshape(
                        B, H, npick * cs, N
                    )
                    sv = torch.gather(vc[:, :, :i], 2, idx).reshape(
                        B, H, npick * cs, N
                    )
                    mask = torch.zeros(
                        hi - lo,
...
class MOBABlock(nn.Module):
    def __init__(self, cfg, layer_id):
        ...
        self.ffn = (
            RWKV_CMix_x070(cfg, layer_id)
            if not cfg.is_moe
            else RWKV_CMix_MoE(cfg, layer_id)
        )
    ...
    def forward(self, x, cmix_state=None, att_state=None, use_cache=False):
        ...
        if checkpoint and att_state is None and not use_cache:
            att_out, _ = torch.utils.checkpoint.checkpoint(
                self.att,
                self.ln1(x),
                use_reentrant=False,
            )
            ...
            att_out, new_att_state = self.att(
                self.ln1(x),
                att_state,
                use_cache,
            )
...
class RWKVBlock(nn.Module):
    ...
    def forward(self, x, v_first, tmix_state=None, cmix_state=None):
        ...
        x = x + xx
        if self.cfg_checkpoint_ffn and self.training and torch.is_grad_enabled():
            fo, cs = torch.utils.checkpoint.checkpoint(
                self.ffn,
                self.ln2(x),
                cmix_state,
                use_reentrant=False,
...
class RWKVXModel(nn.Module):
    def __init__(self, cfg):
        ...
        self.rwkv_blocks = nn.ModuleList(
            [RWKVBlock(cfg, i) for i in range(n_rwkv)]
        )
        self.moba_blocks = nn.ModuleList(
            [MOBABlock(cfg, n_rwkv + i) for i in range(n_moba)]
        )
    ...
    def forward(
        ...
        v_first = None
        if state is None:
            tmix_state, cmix_state, att_state = (
                [None] * len(self.rwkv_blocks),
                [None] * len(self._order),
                [None] * len(self.moba_blocks),
            )
        else:
            tmix_state, cmix_state, att_state = (
                state["tmix"],
                state["cmix"],
                state.get("moba_att", [None] * len(self.moba_blocks)),
            )
        nts, ncs, nas = list(tmix_state), list(cmix_state), list(att_state)
        for pos, (kind, i) in enumerate(self._order):
            if kind == "rwkv":
                x, v_first, ts, cs = self.rwkv_blocks[i](
                    x,
                    v_first,
                    tmix_state[i],
                    cmix_state[pos],
        ...
        new_state = (
            {"tmix": nts, "cmix": ncs, "moba_att": nas}
            if use_cache
            else None
        )
    ...
    def save_pretrained(
        ...
        if include_upstream:
            torch.save(
                {
                    k: v.detach().cpu()
                    for k, v in self.upstream_compatible_state_dict().items()
                },
        ...
        if packed:
            ...
            for path in packed:
                ...
                if sk not in sd or shape not in sd:
                    raise RuntimeError(
                        f"quantized layer {path} is missing shape metadata"
                    )
                parent_path, name = (
