...
impl RwkvModel{
 ...
 pub fn forward_with_tape(&self,token_ids:&[usize])->(Array2<f32>,ModelBackwardTape){let order=self.order.iter().map(|kind|match *kind{BlockKind::Rwkv(index)=>BackwardBlockKind::Rwkv(index),BlockKind::Moba(index)=>BackwardBlockKind::Moba(index)}).collect();let mut tape=ModelBackwardTape::new(order);let(logits,_,_)=self.forward_internal(token_ids,None,Some(&mut tape));(logits,tape)}
