...
use crate::moba_routed_backward;
...
impl MobaBlock {
    pub fn new(channels: usize, head_size: usize, chunk_size: usize, top_k: usize, layer_id: usize, n_layer: usize) -> Self { Self { channels, layer_id, ln1: LayerNorm::new(channels, 1e-5), ln2: LayerNorm::new(channels, 1e-5), att: MobaAttention::new(channels, head_size, chunk_size, top_k), ffn: RwkvCmix::new(channels, layer_id, n_layer) } }
    pub fn forward(&self, x: &Array2<f32>, state: Option<&MobaBlockState>) -> (Array2<f32>, MobaBlockState) {
        let (norm1, att_out, att_k, att_v) = { let n = self.ln1.forward(x); match state { Some(s) if x.nrows() == 1 => { let (y, k, v) = self.att.cache_forward(&n, &s.att_k, &s.att_v); (n, y, k, v) }, _ => { let y = self.att.forward(&n); let h = self.att.heads; let hs = self.att.head_size; let mut k = Array4::zeros((h, x.nrows(), 1, hs)); let mut v = Array4::zeros((h, x.nrows(), 1, hs)); let kp = self.att.key.forward(&n); let vp = self.att.value.forward(&n); for a in 0..h { for t in 0..x.nrows() { for d in 0..hs { k[[a,t,0,d]] = kp[[t,a*hs+d]]; v[[a,t,0,d]] = vp[[t,a*hs+d]]; } } } (n, y, k, v) } } };
        let residual = x + &att_out; let norm2 = self.ln2.forward(&residual); let (ffn_out, cmix_prev) = self.ffn.forward(&norm2, state.map(|s| &s.cmix_prev)); let output = &residual + &ffn_out; (output, MobaBlockState { cmix_prev, att_k, att_v })
    }
    pub fn forward_with_tape(&self, x: &Array2<f32>, state: Option<&MobaBlockState>) -> (Array2<f32>, MobaBlockState, MobaBlockTape) {
        assert!(x.nrows() > 0); let norm1 = self.ln1.forward(x); let q = self.att.receptance.forward(&norm1); let k = self.att.key.forward(&norm1); let v = self.att.value.forward(&norm1); let att_core = self.att.forward_core(&norm1); let att_output = self.att.output.forward(&att_core); let residual = x + &att_output; let norm2 = self.ln2.forward(&residual); let (ffn_output, cmix_prev) = self.ffn.forward(&norm2, state.map(|s| &s.cmix_prev)); let output = &residual + &ffn_output; let mut hk = Array4::zeros((self.att.heads, x.nrows(), 1, self.att.head_size)); let mut hv = Array4::zeros(hk.raw_dim()); for h in 0..self.att.heads { for t in 0..x.nrows() { for d in 0..self.att.head_size { hk[[h,t,0,d]] = k[[t,h*self.att.head_size+d]]; hv[[h,t,0,d]] = v[[t,h*self.att.head_size+d]]; } } } let tape = MobaBlockTape { input: x.clone(), norm1, q, k, v, att_core, att_output, residual, norm2, ffn_output, initial_cmix_prev: state.map(|s| s.cmix_prev.clone()), output: output.clone() }; (output, MobaBlockState { cmix_prev, att_k: hk, att_v: hv }, tape)
    }
    pub fn backward(&self, tape: &MobaBlockTape, grad_output: &Array2<f32>) -> MobaBlockBackward {
        assert_eq!(grad_output.dim(), tape.output.dim()); let grad_ffn = grad_output.clone(); let grad_residual = grad_output.clone(); let ffn = rwkv_cmix_backward::backward(&tape.norm2, tape.initial_cmix_prev.as_ref(), &grad_ffn, &self.ffn.x_k, &self.ffn.key, &self.ffn.value); let (grad_res2, gw2, gb2) = layer_norm_backward::backward(&tape.residual, &ffn.grad_input, &self.ln2.weight, self.ln2.eps); let grad_residual = &grad_residual + &grad_res2; let grad_att = grad_residual.clone(); let grad_core = grad_att.dot(&self.att.output.weight); let mut gq = Array2::zeros(tape.q.raw_dim()); let mut gk = Array2::zeros(tape.k.raw_dim()); let mut gv = Array2::zeros(tape.v.raw_dim()); let scale = (self.att.head_size as f32).sqrt().recip(); for h in 0..self.att.heads { let hs = self.att.head_size; let mut qh = Array2::zeros((tape.q.nrows(),hs)); let mut kh = Array2::zeros((tape.k.nrows(),hs)); let mut vh = Array2::zeros((tape.v.nrows(),hs)); let mut gh = Array2::zeros((grad_core.nrows(),hs)); for t in 0..tape.q.nrows() { for d in 0..hs { qh[[t,d]] = tape.q[[t,h*hs+d]]; kh[[t,d]] = tape.k[[t,h*hs+d]]; vh[[t,d]] = tape.v[[t,h*hs+d]]; gh[[t,d]] = grad_core[[t,h*hs+d]]; } } let b = moba_routed_backward::backward(&qh,&kh,&vh,&gh,self.att.chunk_size,self.att.top_k,scale); for t in 0..tape.q.nrows() { for d in 0..hs { gq[[t,h*hs+d]] = b.grad_q[[t,d]]; gk[[t,h*hs+d]] = b.grad_k[[t,d]]; gv[[t,h*hs+d]] = b.grad_v[[t,d]]; } } } let grad_q_input = gq.dot(&self.att.receptance.weight); let grad_k_input = gk.dot(&self.att.key.weight); let grad_v_input = gv.dot(&self.att.value.weight); let grad_norm1 = &grad_q_input + &grad_k_input + &grad_v_input; let (grad_input_norm,gw1,gb1) = layer_norm_backward::backward(&tape.input,&grad_norm1,&self.ln1.weight,self.ln1.eps); let grad_input = &grad_residual + &grad_input_norm; MobaBlockBackward { grad_input, grad_ln1_weight: gw1, grad_ln1_bias: gb1, grad_ln2_weight: gw2, grad_ln2_bias: gb2, grad_receptance: gq.t().dot(&tape.norm1), grad_key: gk.t().dot(&tape.norm1), grad_value: gv.t().dot(&tape.norm1), grad_output: grad_att.t().dot(&tape.att_core), grad_ffn_x_k: ffn.grad_x_k, grad_ffn_key: ffn.grad_key, grad_ffn_value: ffn.grad_value, grad_cmix_prev: ffn.grad_prev }
    }
    pub fn parameter_count(&self) -> usize { self.ln1.weight.len()+self.ln1.bias.len()+self.ln2.weight.len()+self.ln2.bias.len()+self.att.parameter_count()+self.ffn.parameter_count() }
}
