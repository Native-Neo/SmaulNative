...
impl Tokenizer {
    ...
    pub fn from_vocab_file(path: impl AsRef<Path>) -> Result<Self, String> {
        ...
        let vocab = text
            .lines()
            .map(|x| x.trim_end_matches('\r').to_owned())
            .filter(|x| !x.is_empty())
            .collect();
    ...
    pub fn from_json_file(path: impl AsRef<Path>) -> Result<Self, String> {
        ...
        if let Some(version) = root.get("version").and_then(Value::as_u64) {
            if version < 5 {
                return Err(format!("unsupported SmaulTokenizer version {version}"));
            }
        }
        let object = root
            .get("vocab")
            .and_then(Value::as_object)
            .ok_or("tokenizer JSON has no vocab object")?;
        let mut indexed = Vec::with_capacity(object.len());
        for (token, id) in object {
            indexed.push((
                id.as_u64().ok_or("vocabulary id is not an integer")? as usize,
                token.clone(),
            ));
        }
        ...
        for (expected, (actual, _)) in indexed.iter().enumerate() {
            if expected != *actual {
                return Err("vocabulary ids are not contiguous".into());
            }
        }
        let tokenizer = Self::from_vocab(indexed.into_iter().map(|(_, token)| token).collect());
        for special in [PAD, UNK, BOS, EOS, CAP, UPPER] {
            if !tokenizer.vocab.contains_key(special) {
                return Err(format!("tokenizer is missing required token '{special}'"));
            }
        }
        if let Some(id) = root.get("unk_id").and_then(Value::as_u64) {
            if id as usize != tokenizer.unk_id {
                return Err("unk_id does not match vocab".into());
            }
        }
        Ok(tokenizer)
    }
    ...
    pub fn save_json(&self, path: impl AsRef<Path>) -> Result<(), String> {
        ...
        for (id, token) in self.inverse.iter().enumerate() {
            vocab.insert(token.clone(), Value::from(id));
        }
    ...
    pub fn id_to_token(&self, id: usize) -> Option<&str> {
        self.inverse.get(id).map(String::as_str)
    }
        ...
        for token in lexical_tokens(text) {
            if token.chars().all(char::is_whitespace) {
                for ch in token.chars() {
                    out.push(self.vocab.get(&ch.to_string()).copied().unwrap_or(self.unk_id));
                }
                continue;
            }
            ...
            if let Some(&id) = self.vocab.get(&canonical) {
                if is_capitalized(&token) {
                    out.push(self.cap_id);
                } else if token.chars().all(|c| !c.is_lowercase() && c.is_alphabetic()) {
                    out.push(self.upper_id);
                }
                out.push(id);
                continue;
            }
            ...
            if token.chars().any(|c| ('\u{0900}'..='\u{097F}').contains(&c)) {
                for unit in devanagari_units(&token) {
                    if let Some(&id) = self.vocab.get(&unit) {
                        out.push(id);
                    } else {
...
fn is_capitalized(s: &str) -> bool {
    ...
    match chars.next() {
        Some(first) => first.is_uppercase() && chars.any(char::is_lowercase),
        None => false,
    }
}
...
fn lexical_tokens(text: &str) -> Vec<String> {
    ...
    while i < chars.len() {
        ...
        if ch.is_alphanumeric() || ('\u{0900}'..='\u{097F}').contains(&ch) || ch == '\'' {
            current.push(ch);
            i += 1;
            continue;
        }
...
fn devanagari_units(text: &str) -> Vec<String> {
    ...
    while i < chars.len() {
        ...
        while i < chars.len() {
            let c = chars[i];
            if is_devanagari_mark(c) || c == '\u{200C}' || c == '\u{200D}' {
                unit.push(c);
                i += 1;
                continue;
            }
            if c == '\u{094D}' {
                unit.push(c);
                i += 1;
                if i < chars.len() && ('\u{0900}'..='\u{097F}').contains(&chars[i]) {
                    unit.push(chars[i]);
                    i += 1;
                }
                continue;
...
mod tests {
    ...
    #[test]
    fn encodes_known_words_and_case_markers() {
        assert_eq!(tokenizer().encode("Hello world!"), vec![4, 6, 8, 7, 9]);
    }

    #[test]
    fn decodes_case_markers() {
        assert_eq!(tokenizer().decode(&[4, 6, 8, 7]), "Hello world");
    }

    #[test]
    fn json_round_trip() {
        let tokenizer = tokenizer();
        let path = std::env::temp_dir().join("smaul-tokenizer.json");
        tokenizer.save_json(&path).unwrap();
        let loaded = Tokenizer::from_json_file(&path).unwrap();
        assert_eq!(loaded.vocab_size(), tokenizer.vocab_size());
        assert_eq!(loaded.encode("Hello world!"), tokenizer.encode("Hello world!"));
