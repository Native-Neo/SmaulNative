...
impl Tokenizer {
    pub fn from_vocab(vocab: Vec<String>) -> Self {
        ...
        Self {
            unk_id: id(UNK), pad_id: id(PAD), bos_id: id(BOS), eos_id: id(EOS),
            cap_id: id(CAP), upper_id: id(UPPER), vocab: map, inverse: vocab,
        }
    }
        ...
        for token in lexical_tokens(text) {
            ...
            for unit in devanagari_units(&token) {
                if let Some(&id) = self.vocab.get(&unit) { out.push(id); }
                    ...
                    for ch in unit.chars() { out.push(self.vocab.get(&ch.to_string()).copied().unwrap_or(self.unk_id)); }
                }
            }
        }
        out
...
fn lexical_tokens(text: &str) -> Vec<String> {
    ...
    let flush = |out: &mut Vec<String>, current: &mut String| {
        if !current.is_empty() { out.push(std::mem::take(current)); }
    };
...
fn devanagari_units(text: &str) -> Vec<String> {
    ...
    while i < chars.len() {
        ...
        while i < chars.len() {
            let c = chars[i];
            if c.is_mark() || c == '\u{200C}' || c == '\u{200D}' { unit.push(c); i += 1; continue; }
            if c == '\u{094D}' {
                unit.push(c); i += 1;
...
mod tests {
    ...
    fn tokenizer() -> Tokenizer {
        Tokenizer::from_vocab(vec![PAD.into(), UNK.into(), BOS.into(), EOS.into(), CAP.into(), UPPER.into(), "hello".into(), "world".into(), " ".into(), "!".into(), "नमस्ते".into()])
    }

    #[test]
    fn encodes_known_words_and_case_markers() {
        let tok = tokenizer();
        assert_eq!(tok.encode("Hello world!"), vec![4, 6, 8, 7, 9]);
    }

    #[test]
    fn decodes_case_markers() {
        let tok = tokenizer();
        assert_eq!(tok.decode(&[4, 6, 8, 7]), "Hello world");
    }

    #[test]
    fn unknown_text_is_represented() {
        let tok = tokenizer();
        assert!(!tok.encode("xyz").is_empty());
    }
}
