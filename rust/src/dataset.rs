...
impl TextStream {
    pub fn open(path: impl AsRef<Path>, tokenizer: Tokenizer, ctx_len: usize) -> Result<Self, String> {
        Self::open_jsonl(path, tokenizer, ctx_len, None)
    }
    ...
    pub fn open_jsonl(
        ...
        tokenizer: Tokenizer,
        ctx_len: usize,
        text_field: Option<impl Into<String>>,
    ) -> Result<Self, String> {
        if ctx_len == 0 {
            return Err("ctx_len must be greater than zero".into());
        }
        ...
        Ok(Self {
            ...
            buffer: Vec::new(),
            path,
            jsonl_text_field: text_field.map(Into::into),
        })
    }

    fn read_text(&self, line: &str) -> Result<String, String> {
        if self.jsonl_text_field.is_none() {
            return Ok(line.trim_end_matches(['\n', '\r']).to_owned());
        }
        let value: serde_json::Value = serde_json::from_str(line)
            .map_err(|e| format!("invalid JSONL record {}: {e}", self.record))?;
        let field = self.jsonl_text_field.as_ref().unwrap();
        value.get(field)
            .and_then(serde_json::Value::as_str)
            .map(str::to_owned)
            .ok_or_else(|| format!("JSONL record {} has no string field '{field}'", self.record))
    }
    ...
    pub fn next_batch(&mut self) -> Result<Option<TokenBatch>, String> {
        loop {
            if self.buffer.len() >= self.ctx_len + 1 {
                ...
                return Ok(Some(TokenBatch {
                    input,
                    target,
                    position: DatasetPosition { file: self.path.clone(), record: self.record },
                }));
            }
            ...
            if self.reader.read_line(&mut line).map_err(|e| e.to_string())? == 0 {
                ...
                while self.buffer.len() < self.ctx_len + 1 {
    ...
    pub fn position(&self) -> DatasetPosition {
        DatasetPosition { file: self.path.clone(), record: self.record }
    }
...
pub fn discover_files(dir: impl AsRef<Path>) -> Result<Vec<PathBuf>, String> {
    ...
    let entries = std::fs::read_dir(dir.as_ref()).map_err(|e| e.to_string())?;
    for entry in entries {
        let path = entry.map_err(|e| e.to_string())?.path();
        if path.is_file() {
            if matches!(path.extension().and_then(|x| x.to_str()), Some("txt" | "text" | "jsonl")) {
                files.push(path);
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn streams_fixed_length_training_pairs() {
        let path = std::env::temp_dir().join("smaul-dataset.txt");
        fs::write(&path, "a b a b a b").unwrap();
        ...
        assert_eq!(batch.position.record, 1);
        let _ = fs::remove_file(path);
    }

    #[test]
    fn reads_jsonl_text_field() {
        let path = std::env::temp_dir().join("smaul-dataset.jsonl");
        fs::write(&path, "{\"text\":\"a b a\"}\n{\"text\":\"b a\"}\n").unwrap();
        let mut stream = TextStream::open_jsonl(&path, tokenizer(), 2, Some("text")).unwrap();
