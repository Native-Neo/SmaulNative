...
fn fp8_e4m3(code: u8) -> f32 {
    if code == 0x7f { return f32::NAN; }
    if code & 0x7f == 0 { return if code & 0x80 != 0 { -0.0 } else { 0.0 }; }
    let sign = if code & 0x80 != 0 { -1.0 } else { 1.0 };
    let exponent = ((code >> 3) & 0x0f) as i32 - 7;
    let mantissa = (code & 0x07) as f32;
    sign * 2.0_f32.powi(exponent) * (1.0 + mantissa / 8.0)
}
...
pub fn fake_quantize(x:&Array2<f32>,bits:Bits)->Array2<f32>{match bits{Bits::Fp8=>x.mapv(fp8_e4m3_value),Bits::Fp2|Bits::Fp4=>{let(codes,scales)=quantize_codes(x,bits);dequantize(&codes,&scales,bits)}}}
...
pub fn quantize_weight(weight:&Array2<f32>,bits:Bits)->(Vec<u8>,Vec<f32>){match bits{Bits::Fp8=>{let bytes=weight.iter().map(|&v|fp8_e4m3_code(v)).collect();(bytes,vec![1.0])},Bits::Fp2|Bits::Fp4=>{let(codes,scales)=quantize_codes(weight,bits);(pack_lowbit(codes.as_slice().unwrap(),bits),scales)}}}
pub fn dequantize_weight(packed:&[u8],scales:&[f32],shape:(usize,usize),bits:Bits)->Array2<f32>{match bits{Bits::Fp8=>{assert!(packed.len()>=shape.0*shape.1);let values=(0..shape.0*shape.1).map(|i|fp8_e4m3(packed[i])).collect();Array2::from_shape_vec(shape,values).unwrap()},Bits::Fp2|Bits::Fp4=>{let codes=unpack_lowbit(packed,shape.0*shape.1,bits);let levels=bits.levels();Array2::from_shape_fn(shape,|(r,c)|levels[codes[r*shape.1+c]as usize]*scales[c])}}}
