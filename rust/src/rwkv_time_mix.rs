...
impl RwkvTimeMix {
    ...
    pub fn new_with_qat(channels: usize, heads: usize, layer_id: usize, n_layer: usize, qat_bits: u8) -> Self {
        assert_eq!(channels % heads, 0); assert!(matches!(qat_bits,0|2|4|8)); let head_size=channels/heads;
        let r0=layer_id as f32/n_layer.saturating_sub(1).max(1) as f32; let r1=1.0-layer_id as f32/n_layer as f32;
        let dd=(1.8*(channels as f32).sqrt()/32.0).round().max(1.0) as usize*32; let dm=(1.3*(channels as f32).sqrt()/32.0).round().max(1.0) as usize*32; let dg=(0.6*(channels as f32).powf(0.8)/32.0).round().max(1.0) as usize*32;
        let mut x_r=Array1::zeros(channels);let mut x_w=Array1::zeros(channels);let mut x_k=Array1::zeros(channels);let mut x_v=Array1::zeros(channels);let mut x_a=Array1::zeros(channels);let mut x_g=Array1::zeros(channels);let mut w0=Array1::zeros(channels);
        for c in 0..channels{let d=if channels>1{c as f32/(channels-1) as f32}else{0.0};let dc=c as f32/channels as f32;x_r[c]=1.0-dc.powf(0.2*r1);x_w[c]=1.0-dc.powf(0.9*r1);x_k[c]=1.0-(dc.powf(0.9*r1)+0.4*r0);x_v[c]=1.0-(dc.powf(0.4*r1)+0.6*r0);x_a[c]=1.0-dc.powf(0.9*r1);x_g[c]=1.0-dc.powf(0.2*r1);w0[c]=-7.0+5.0*d.powf(0.85+r0.sqrt())+0.5;}
        let w1=Array2::zeros((channels,dd));let a1=Array2::zeros((channels,dd));let g1=Array2::zeros((channels,dg));let v1=(layer_id>0).then(||Array2::zeros((channels,dm)));
        let w2_gain=(dd as f32/channels as f32).sqrt().max(1.0)*0.1;let a2_gain=w2_gain;let g2_gain=(dg as f32/channels as f32).sqrt().max(1.0)*0.1;let v2_gain=(dm as f32/channels as f32).sqrt().max(1.0)*0.1;
        let w2=orthogonal(dd,channels,w2_gain,0x5752_5637_3030^layer_id as u64);let a2=orthogonal(dd,channels,a2_gain,0x4132_3030_3030^layer_id as u64);let g2=orthogonal(dg,channels,g2_gain,0x4732_3030_3030^layer_id as u64);let v2=(layer_id>0).then(||orthogonal(dm,channels,v2_gain,0x5632_3030_3030^layer_id as u64));
        let lim=0.5/(channels as f32).sqrt();let receptance=uniform(channels,channels,-lim,lim,0x5253_3030_3030^layer_id as u64);let key=uniform(channels,channels,-0.05/(channels as f32).sqrt(),0.05/(channels as f32).sqrt(),0x4b59_3030_3030^layer_id as u64);let value=uniform(channels,channels,-lim,lim,0x5641_3030_3030^layer_id as u64);let output=Array2::zeros((channels,channels));
        let mut s=Self{channels,heads,head_size,x_r,x_w,x_k,x_v,x_a,x_g,w0,w1,w2,a1,a2,v1,v2,g1,g2,a0:Array1::zeros(channels),v0:(layer_id>0).then(||Array1::ones(channels)),k_k:Array1::from_elem(channels,0.85),k_a:Array1::ones(channels),r_k:Array2::zeros((heads,head_size)),receptance,key,value,output,ln_x:GroupNorm::new(channels,heads,1e-5*8.0*8.0),qat_bits,q_w1:None,q_w2:None,q_a1:None,q_a2:None,q_v1:None,q_v2:None,q_g1:None,q_g2:None,q_receptance:None,q_key:None,q_value:None,q_output:None}; s.refresh_quantized(); s
    }
    ...
    pub fn forward_with_tape(&self,x:&Array2<f32>,state:Option<Array4<f32>>,prev:Option<Array1<f32>>,v_first:Option<Array2<f32>>)->(Array2<f32>,Array4<f32>,Array1<f32>,Array2<f32>,RwkvTimeMixTape){
        assert!(x.nrows()>0);let zero=Array1::zeros(self.channels);let prev_ref=prev.as_ref().unwrap_or(&zero);let initial=state.unwrap_or_else(||Array4::zeros((1,self.heads,self.head_size,self.head_size)));let mut tape=RwkvTimeMixTape::new(x.clone(),prev_ref.clone(),initial.clone(),self.w1.ncols(),self.v1.as_ref().map(|v|v.ncols()),self.g1.ncols());
        tape.xr=mix(x,prev_ref,&self.x_r);tape.xw=mix(x,prev_ref,&self.x_w);tape.xk=mix(x,prev_ref,&self.x_k);tape.xv=mix(x,prev_ref,&self.x_v);tape.xa=mix(x,prev_ref,&self.x_a);tape.xg=mix(x,prev_ref,&self.x_g);
        tape.r=Self::project(&tape.xr,&self.receptance,&self.q_receptance);tape.w_hidden=Self::project(&tape.xw,&self.w1,&self.q_w1).mapv(|v|v.tanh());tape.g_decay=Self::project(&tape.w_hidden,&self.w2,&self.q_w2);tape.k=Self::project(&tape.xk,&self.key,&self.q_key);tape.v_base=Self::project(&tape.xv,&self.value,&self.q_value);tape.v=tape.v_base.clone();tape.a_hidden=Self::project(&tape.xa,&self.a1,&self.q_a1);tape.a=Self::project(&tape.a_hidden,&self.a2,&self.q_a2);for t in 0..tape.a.nrows(){for c in 0..self.channels{tape.a[[t,c]]=sigmoid(self.a0[c]+tape.a[[t,c]]);}}
        tape.g_hidden=Self::project(&tape.xg,&self.g1,&self.q_g1).mapv(sigmoid);tape.g=Self::project(&tape.g_hidden,&self.g2,&self.q_g2);tape.v_first=v_first.unwrap_or_else(||tape.v.clone());
        if let(Some(v1),Some(v2),Some(v0))=(&self.v1,&self.v2,&self.v0){let correction=Self::project(&Self::project(&tape.xv,v1,&self.q_v1),v2,&self.q_v2);let mut gate=Array2::zeros(tape.v.dim());for t in 0..tape.v.nrows(){for c in 0..self.channels{gate[[t,c]]=sigmoid(v0[c]+correction[[t,c]]);tape.v[[t,c]]+=(tape.v_first[[t,c]]-tape.v[[t,c]])*gate[[t,c]];}}tape.v_correction=Some(correction);tape.v_gate=Some(gate);}
        tape.kk_pre_norm=tape.k.clone();tape.kk=tape.k.clone();for t in 0..tape.kk.nrows(){for h in 0..self.heads{let start=h*self.head_size;let mut norm=0.0;for i in 0..self.head_size{let z=tape.kk[[t,start+i]]*self.k_k[start+i];norm+=z*z;}let inv=(norm+1e-12).sqrt().recip();for i in 0..self.head_size{tape.kk[[t,start+i]]*=self.k_k[start+i]*inv;}}}
        tape.k_mod=tape.k.clone();for t in 0..tape.k_mod.nrows(){for c in 0..self.channels{tape.k_mod[[t,c]]*=1.0+(tape.a[[t,c]]-1.0)*self.k_a[c];}}tape.w=Array2::zeros(x.raw_dim());for t in 0..x.nrows(){for c in 0..self.channels{tape.w[[t,c]]=(-0.606531*sigmoid(self.w0[c]+tape.g_decay[[t,c]])).exp();}}
        let(next_state,y)=wkv::run(initial,&tape.w,&tape.k_mod,&tape.v,&tape.kk,&tape.a,&tape.r,self.heads,self.head_size);tape.y=y;tape.normalized=self.ln_x.forward(&tape.y);tape.correction=Array2::zeros(tape.y.raw_dim());for t in 0..tape.normalized.nrows(){for h in 0..self.heads{let start=h*self.head_size;let mut corr=0.0;for i in 0..self.head_size{let c=start+i;corr+=tape.r[[t,c]]*tape.k_mod[[t,c]]*self.r_k[[h,i]];}for i in 0..self.head_size{tape.correction[[t,start+i]]=corr*tape.v[[t,start+i]];}}}tape.gated=&tape.normalized+&tape.correction;for t in 0..tape.gated.nrows(){for c in 0..self.channels{tape.gated[[t,c]]*=tape.g[[t,c]];}}tape.output=Self::project(&tape.gated,&self.output,&self.q_output);let last=x.row(x.nrows()-1).to_owned();(tape.output.clone(),next_state,last,tape.v_first.clone(),tape)
    }
