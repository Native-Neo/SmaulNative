...
impl RwkvTimeMix {
    pub fn new(channels: usize, heads: usize, layer_id: usize, n_layer: usize) -> Self {
        ...
        let w1 = Array2::zeros((channels, dd)); let a1 = Array2::zeros((channels, dd)); let g1 = Array2::zeros((channels, dg));
        let v1 = (layer_id > 0).then(|| Array2::zeros((channels, dm)));
        let w2_gain = (dd as f32 / channels as f32).sqrt().max(1.0) * 0.1;
        let a2_gain = w2_gain; let g2_gain = (dg as f32 / channels as f32).sqrt().max(1.0) * 0.1; let v2_gain = (dm as f32 / channels as f32).sqrt().max(1.0) * 0.1;
        ...
        Self { channels, heads, head_size, x_r, x_w, x_k, x_v, x_a, x_g, w0, w1, w2, a1, a2, v1, v2, g1, g2,
            ...
            receptance, key, value, output: Array2::zeros((channels, channels)), ln_x: GroupNorm::new(channels, heads, 1e-5 * 8.0 * 8.0) }
    }
    ...
    pub fn forward_with_tape(&self, x: &Array2<f32>, state: Option<Array4<f32>>, prev: Option<Array1<f32>>, v_first: Option<Array2<f32>>) -> (Array2<f32>, Array4<f32>, Array1<f32>, Array2<f32>, RwkvTimeMixTape) {
        ...
        let mut tape = RwkvTimeMixTape::new(x.clone(), prev_ref.clone(), initial.clone(), self.w1.ncols(), self.v1.as_ref().map(|v| v.ncols()), self.g1.ncols());
        tape.xr = mix(x, prev_ref, &self.x_r); tape.xw = mix(x, prev_ref, &self.x_w); tape.xk = mix(x, prev_ref, &self.x_k);
        tape.xv = mix(x, prev_ref, &self.x_v); tape.xa = mix(x, prev_ref, &self.x_a); tape.xg = mix(x, prev_ref, &self.x_g);
        tape.r = Self::project(&tape.xr, &self.receptance);
        tape.w_hidden = Self::project(&tape.xw, &self.w1).mapv(|v| v.tanh()); tape.g_decay = Self::project(&tape.w_hidden, &self.w2);
        tape.k = Self::project(&tape.xk, &self.key); tape.v_base = Self::project(&tape.xv, &self.value); tape.v = tape.v_base.clone();
        tape.a_hidden = Self::project(&tape.xa, &self.a1);
        tape.a = Self::project(&tape.a_hidden, &self.a2); for t in 0..tape.a.nrows() { for c in 0..self.channels { tape.a[[t,c]] = sigmoid(self.a0[c] + tape.a[[t,c]]); }}
        tape.g_hidden = Self::project(&tape.xg, &self.g1).mapv(sigmoid); tape.g = Self::project(&tape.g_hidden, &self.g2);
        tape.v_first = v_first.unwrap_or_else(|| tape.v.clone());
        if let (Some(v1), Some(v2), Some(v0)) = (&self.v1, &self.v2, &self.v0) {
            let correction = Self::project(&Self::project(&tape.xv, v1), v2); let mut gate = Array2::zeros(tape.v.dim());
            ...
            tape.v_correction = Some(correction); tape.v_gate = Some(gate);
        }
        tape.kk_pre_norm = tape.k.clone(); tape.kk = tape.k.clone();
        for t in 0..tape.kk.nrows() { for h in 0..self.heads { let start=h*self.head_size; let mut norm=0.0; for i in 0..self.head_size { let z=tape.kk[[t,start+i]]*self.k_k[start+i]; norm+=z*z; } let inv=(norm+1e-12).sqrt().recip(); for i in 0..self.head_size { tape.kk[[t,start+i]]*=self.k_k[start+i]*inv; } }}
        tape.k_mod = tape.k.clone(); for t in 0..tape.k_mod.nrows() { for c in 0..self.channels { tape.k_mod[[t,c]]*=1.0+(tape.a[[t,c]]-1.0)*self.k_a[c]; }}
        tape.w=Array2::zeros(x.raw_dim()); for t in 0..x.nrows() { for c in 0..self.channels { tape.w[[t,c]]=(-0.606531*sigmoid(self.w0[c]+tape.g_decay[[t,c]])).exp(); }}
        let (next_state,y)=wkv::run(initial,&tape.w,&tape.k_mod,&tape.v,&tape.kk,&tape.a,&tape.r,self.heads,self.head_size); tape.y=y;
        tape.normalized=self.ln_x.forward(&tape.y); tape.correction=Array2::zeros(tape.y.raw_dim());
        for t in 0..tape.normalized.nrows() { for h in 0..self.heads { let start=h*self.head_size; let mut corr=0.0; for i in 0..self.head_size { let c=start+i; corr+=tape.r[[t,c]]*tape.k_mod[[t,c]]*self.r_k[[h,i]]; } for i in 0..self.head_size { tape.correction[[t,start+i]]=corr*tape.v[[t,start+i]]; } }}
        tape.gated=&tape.normalized+&tape.correction; for t in 0..tape.gated.nrows() { for c in 0..self.channels { tape.gated[[t,c]]*=tape.g[[t,c]]; }}
        tape.output=tape.gated.dot(&self.output); let last=x.row(x.nrows()-1).to_owned();
...
mod tests {
    use super::*;
    #[test]
    fn x070_shapes() { let layer=RwkvTimeMix::new(16,2,0,4); let x=Array2::ones((3,16)); let (out,state,last,first)=layer.forward(&x,None,None,None); assert_eq!(out.shape(),&[3,16]); assert_eq!(state.shape(),&[1,2,8,8]); assert_eq!(last.len(),16); assert_eq!(first.shape(),&[3,16]); }
    #[test]
    fn tape_matches_forward_output() { let layer=RwkvTimeMix::new(16,2,1,4); let x=Array2::ones((3,16)); let (a,_,_,_,tape)=layer.forward_with_tape(&x,None,None,None); let (b,_,_,_)=layer.forward(&x,None,None,None); assert!(a.iter().zip(b.iter()).all(|(x,y)|(x-y).abs()<1e-6)); assert_eq!(tape.w_hidden.ncols(),layer.w1.ncols()); }
}
