...
impl Default for RwkvXConfig {
    fn default() -> Self { Self { vocab_size: 65530, n_embd: 832, n_layer: 17, head_size: 64, n_moba_layer: 5, moba_chunk_size: 512, moba_topk: 4, dropout: 0.0, head_size_divisor: 8, ctx_len_hint: 2048, wkv_chunk_size: 64, checkpoint_ffn: true, is_moe: false, num_experts: 1, num_experts_per_tok: 1 } }
}
impl RwkvXConfig {
    pub fn validate(&self) -> Result<(), String> {
        ...
        } else if self.num_experts != 1 || self.num_experts_per_tok != 1 { return Err("non-MoE config must use num_experts=1 and num_experts_per_tok=1".into()); }
        Ok(())
    }
    ...
    pub fn from_model_config(config: &RwkvModelConfig) -> Self { Self { vocab_size: config.vocab_size, n_embd: config.n_embd, n_layer: config.n_layer, head_size: config.head_size, n_moba_layer: config.n_moba_layer, moba_chunk_size: config.moba_chunk_size, moba_topk: config.moba_topk, head_size_divisor: config.head_size_divisor, is_moe: config.is_moe, num_experts: config.num_experts, num_experts_per_tok: config.num_experts_per_tok, ..Self::default() } }
...
#[cfg(test)]
mod tests { use super::*; #[test] fn config_round_trip(){let p=std::env::temp_dir().join("smaul-rwkv-config.json");let c=RwkvXConfig::default();c.save(&p).unwrap();let d=RwkvXConfig::load(&p).unwrap();assert_eq!(d.vocab_size,c.vocab_size);assert_eq!(d.n_embd,c.n_embd);assert_eq!(d.n_layer,c.n_layer);assert_eq!(d.n_moba_layer,c.n_moba_layer);let _=std::fs::remove_file(p);} #[test] fn model_config_preserves_architecture(){let c=RwkvXConfig::default();let m=c.to_model_config();assert_eq!(m.vocab_size,c.vocab_size);assert_eq!(m.n_embd,c.n_embd);assert_eq!(m.n_layer,c.n_layer);assert_eq!(m.n_moba_layer,c.n_moba_layer);assert_eq!(m.is_moe,c.is_moe);assert_eq!(m.num_experts,c.num_experts);assert_eq!(m.num_experts_per_tok,c.num_experts_per_tok);} #[test] fn target_parameter_search_matches_layer_range(){let c=RwkvXConfig::config_for_target_params(257_000_000,65_530,832,5,64).unwrap();assert!((4..80).contains(&c.n_layer));assert_eq!(c.n_embd%c.head_size,0);assert!(c.n_moba_layer<c.n_layer);} #[test] fn target_parameter_search_rejects_bad_head_size(){assert!(RwkvXConfig::config_for_target_params(1000,64,10,1,3).is_err());} #[test] fn validation_rejects_invalid_moe(){let mut c=RwkvXConfig::default();c.is_moe=true;c.num_experts=4;c.num_experts_per_tok=5;assert!(c.validate().is_err());} #[test] fn validation_rejects_invalid_moba_layout(){let mut c=RwkvXConfig::default();c.n_moba_layer=c.n_layer;assert!(c.validate().is_err());} }
