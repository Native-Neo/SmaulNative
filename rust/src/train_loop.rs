...
use crate::training_state::TrainingState;
use ndarray::Array1;
use std::path::Path;

pub struct TrainingConfig { pub max_steps: usize, pub log_every: usize, pub save_every: usize, pub max_grad_norm: Option<f32> }
impl Default for TrainingConfig { fn default()->Self{Self{max_steps:0,log_every:1,save_every:0,max_grad_norm:Some(1.0)}} }
...
impl TrainingRunner {
 ...
 pub fn save_state(&self,path:impl AsRef<Path>)->Result<(),String>{crate::training_state::CheckpointBundle::new(self.state.clone()).save(path)}
}
...
mod tests{
 ...
 #[test]fn trains_native_stream(){let path=std::env::temp_dir().join("smaul-train.txt");fs::write(&path,"a b a b a b a b").unwrap();let mut stream=TextStream::open(&path,tokenizer(),3).unwrap();let mut model=RwkvModel::new(RwkvModelConfig::new(9,8,1,4),42);let mut runner=TrainingRunner::new(&model,1e-4);let state=runner.run_stream(&mut model,&mut stream,&TrainingConfig{max_steps:1,..Default::default()}).unwrap();assert_eq!(state.step,1);assert!(state.tokens_seen>=3);let _=fs::remove_file(path);}
 #[test]fn trains_multiple_files(){let dir=std::env::temp_dir().join("smaul-train-multi");fs::create_dir_all(&dir).unwrap();fs::write(dir.join("a.txt"),"a b a b a b").unwrap();fs::write(dir.join("b.txt"),"b a b a b a").unwrap();let mut stream=MultiFileTextStream::open_discovered(&dir,tokenizer(),3).unwrap();let mut model=RwkvModel::new(RwkvModelConfig::new(9,8,1,4),42);let mut runner=TrainingRunner::new(&model,1e-4);let state=runner.run_multi_stream(&mut model,&mut stream,&TrainingConfig{max_steps:2,..Default::default()}).unwrap();assert_eq!(state.step,2);let _=fs::remove_dir_all(dir);}
}