...
impl TrainingRunner {
    pub fn new(model: &RwkvModel, learning_rate: f32) -> Self {
        Self {
            optimizer: TrainStep::new(model, learning_rate),
    ...
    pub fn run_multi_stream(
        &mut self,
        model: &mut RwkvModel,
        stream: &mut MultiFileTextStream,
        config: &TrainingConfig,
    ) -> Result<TrainingState, String> {
        self.run_batches(model, config, || stream.next_batch())
    }
    ...
    where
        F: FnMut() -> Result<Option<crate::dataset::TokenBatch>, String>,
    {
        if config.log_every == 0 {
            return Err("log_every must be greater than zero".into());
        }
        if config.save_every > 0 && config.checkpoint_dir.is_none() {
            return Err("checkpoint_dir is required when save_every is enabled".into());
        }
        ...
        while config.max_steps == 0 || self.state.step < config.max_steps {
            ...
            if self.state.step % config.log_every == 0 {
                println!(
                    "step {} | loss {:.6} | tokens {}",
                    self.state.step, result.loss, self.state.tokens_seen
                );
            }

            if config.save_every > 0 && self.state.step % config.save_every == 0 {
                self.save_checkpoint(model, config.checkpoint_dir.as_ref().unwrap())?;
            }
    ...
    fn save_checkpoint(&self, model: &RwkvModel, directory: &Path) -> Result<(), String> {
        ...
        let model_path = directory.join(format!("model-{step}.safetensors"));
        let state_path = directory.join(format!("training-{step}.json"));
        crate::model_saver::save_model_safetensors(model, &model_path)?;
        self.state.clone().save(&state_path)?;
        Ok(())
    }

    pub fn save_state(&self, path: impl AsRef<Path>) -> Result<(), String> {
        self.state.clone().save(path)
    }
}
...
mod tests {
    ...
    fn trains_native_stream() {
        ...
        let state = runner
            .run_stream(&mut model, &mut stream, &TrainingConfig { max_steps: 1, ..Default::default() })
            .unwrap();
    ...
    fn trains_multiple_files() {
        ...
        let mut runner = TrainingRunner::new(&model, 1e-4);
        let state = runner
            .run_multi_stream(&mut model, &mut stream, &TrainingConfig { max_steps: 2, ..Default::default() })
            .unwrap();
    ...
    #[test]
    fn saves_model_and_training_state() {
        let input = std::env::temp_dir().join("smaul-checkpoint-input.txt");
        let dir = std::env::temp_dir().join("smaul-checkpoint-output");
        fs::write(&input, "a b a b a b").unwrap();
        let mut stream = TextStream::open(&input, tokenizer(), 3).unwrap();
        let mut model = RwkvModel::new(RwkvModelConfig::new(9, 8, 1, 4), 42);
        let mut runner = TrainingRunner::new(&model, 1e-4);
        ...
        runner.run_stream(&mut model, &mut stream, &config).unwrap();
        assert!(dir.join("model-1.safetensors").exists());
        assert!(dir.join("training-1.json").exists());
