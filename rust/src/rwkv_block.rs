...
impl RwkvBlock {
    ...
    pub fn forward_with_full_tape(
        &self,
        x: &Array2<f32>,
        state: Option<&RwkvBlockState>,
        v_first: Option<&Array2<f32>>,
    ) -> (Array2<f32>, RwkvBlockState, RwkvBlockFullTape) {
        assert_eq!(x.ncols(), self.channels);
        let input = x.clone();
        let (ln0_input, ln0_output) = match &self.ln0 {
            Some(norm) => (Some(input.clone()), Some(norm.forward(&input))),
            None => (None, None),
        };
        ...
        let time_prev = state.map(|s| s.time_prev.clone());
        let inherited_v_first = v_first.cloned().or_else(|| state.and_then(|s| s.v_first.clone()));
        let (time_output, next_time_state, next_time_prev, next_v_first, time_tape) = self.time_mix.forward_with_tape(
            &ln1_output, time_state, time_prev, inherited_v_first,
        );
        ...
        for t in 0..cmix_mixed.nrows() {
            for c in 0..self.channels {
                let previous = if t == 0 { cmix_prev[c] } else { ln2_output[[t - 1, c]] };
                cmix_mixed[[t, c]] = ln2_output[[t, c]] + (previous - ln2_output[[t, c]]) * self.cmix.x_k[c];
            }
        }
        ...
        let output = &residual + &cmix_output;
        let next_state = RwkvBlockState {
            time_state: next_time_state,
            time_prev: next_time_prev,
            cmix_prev: ln2_output.row(ln2_output.nrows() - 1).to_owned(),
            v_first: Some(next_v_first),
        };
        let mut tape = RwkvBlockFullTape::new(input, time_tape, next_state.clone());
        tape.initial_state = state.map(|s| RwkvBlockState {
            time_state: s.time_state.clone(), time_prev: s.time_prev.clone(), cmix_prev: s.cmix_prev.clone(), v_first: s.v_first.clone(),
        });
        tape.ln0_input = ln0_input;
        tape.ln0_output = ln0_output;
        tape.ln1_input = ln1_input;
        tape.ln1_output = ln1_output;
        tape.residual = residual;
        tape.ln2_input = ln2_input;
        tape.ln2_output = ln2_output;
        tape.cmix_input = tape.ln2_output.clone();
        tape.cmix_prev = cmix_prev;
        tape.cmix_mixed = cmix_mixed;
        tape.cmix_pre = cmix_pre;
        tape.cmix_hidden = cmix_hidden;
        tape.cmix_output = cmix_output;
        tape.output = output.clone();
...
mod tests {
    ...
    #[test]
    fn full_tape_matches_forward_shape() {
        let block = RwkvBlock::new(16, 2, 0, 4);
        let x = Array2::<f32>::ones((3, 16));
        let (a, _, tape) = block.forward_with_full_tape(&x, None, None);
        let (b, _) = block.forward(&x, None, None);
        assert_eq!(a.dim(), b.dim());
        assert!(a.iter().zip(b.iter()).all(|(x, y)| (x - y).abs() < 1e-6));
        assert_eq!(tape.cmix_pre.dim(), (3, 64));
    }
