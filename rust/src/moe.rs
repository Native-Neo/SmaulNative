...
impl MoeRouter {
    pub fn new(input_size: usize, num_experts: usize, top_k: usize, seed: u64) -> Self {
        ...
        Self { input_size, num_experts, top_k, weight: uniform(num_experts, input_size, -scale, scale, seed) }
    }
    ...
    pub fn route(&self, x: &Array2<f32>) -> Array2<f32> {
        ...
        for row in 0..logits.nrows() {
            ...
            for &i in selected { gates[[row, i]] = (logits[[row, i]] - max).exp() / sum; }
        }
        gates
    }
...
impl MoeCmix {
    ...
    pub fn with_qat_3bit(mut self, enabled: bool) -> Self {
        for expert in &mut self.experts { expert.qat_3bit = enabled; }
        self
    }
    ...
    pub fn from_experts(experts: Vec<RwkvCmix>, router: MoeRouter) -> Result<Self, String> {
        ...
        if experts.iter().any(|expert| expert.channels != router.input_size) { return Err("expert channel count does not match router input size".into()); }
    ...
    pub fn forward(&self, x: &Array2<f32>, prev: Option<&Array1<f32>>) -> (Array2<f32>, Array1<f32>) {
        ...
        for expert_id in 0..self.experts.len() {
            ...
            for (local_row, &row) in rows.iter().enumerate() {
                ...
                for col in 0..x.ncols() { output[[row, col]] += gate * expert_output[[local_row, col]]; }
            }
        }
...
mod tests {
    use super::*;
    use ndarray::Array2;
    #[test] fn router_selects_exact_top_k() { let router = MoeRouter::from_weight(Array2::from_shape_vec((3, 2), vec![10.0, 0.0, 0.0, 10.0, 1.0, 1.0]).unwrap(), 2); let gates = router.route(&Array2::from_shape_vec((1, 2), vec![1.0, 0.0]).unwrap()); assert_eq!(gates.iter().filter(|&&v| v > 0.0).count(), 2); assert!((gates.sum() - 1.0).abs() < 1e-6); }
    #[test] fn cmix_moe_preserves_shape() { let moe = MoeCmix::new(8, 0, 4, 3, 2, 7); let x = Array2::<f32>::ones((4, 8)); let (y, last) = moe.forward(&x, None); assert_eq!(y.dim(), (4, 8)); assert_eq!(last.len(), 8); assert_eq!(moe.parameter_count(), moe.router.parameter_count() + moe.experts.iter().map(RwkvCmix::parameter_count).sum::<usize>()); }
    #[test] fn experts_have_distinct_initialization() { let moe = MoeCmix::new(8, 0, 4, 3, 2, 7); assert_ne!(moe.experts[0].key, moe.experts[1].key); assert_ne!(moe.experts[1].key, moe.experts[2].key); }
    #[test] fn rejects_invalid_expert_router_pair() { let experts = vec![RwkvCmix::new(8, 0, 4)]; let router = MoeRouter::new(8, 2, 1, 1); assert!(MoeCmix::from_experts(experts, router).is_err()); }
    #[test] fn qat_reaches_every_expert() { let moe = MoeCmix::new(8, 0, 4, 3, 2, 7).with_qat_3bit(true); assert!(moe.experts.iter().all(|expert| expert.qat_3bit)); }
}
