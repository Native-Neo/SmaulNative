...
pub fn backward_with_state_grads(model: &RwkvModel, token_ids: &[usize], tape: &ModelBackwardTape, logits_grad: &Array2<f32>, grad_next_states: Option<&[Option<RwkvBlockState>]>) -> RwkvModelBackward {
    ...
    let normalized=tape.normalized.as_ref().expect("missing normalized output"); let ln_input=tape.ln_input.as_ref().expect("missing layer norm input"); let logits=tape.logits.as_ref().expect("missing logits"); assert_eq!(logits_grad.dim(),logits.dim());
    ...
    for slot in (0..model.config.n_layer).rev() {
        match tape.block_order[slot] {
            BackwardBlockKind::Rwkv(index)=>{
                let block_tape=tape.rwkv_tapes[slot].as_ref().expect("missing RWKV block tape"); let next=grad_next_states.and_then(|all|all[index].as_ref());
                let block_grad=rwkv_block_backward_full::backward(&model.rwkv_blocks[index],block_tape,&grad,next.map(|s|&s.time_state),next.map(|s|&s.time_prev),next.map(|s|&s.cmix_prev),grad_v_first.as_ref());
                grad=block_grad.grad_input.clone(); grad_initial_states[index]=Some(RwkvBlockState{time_state:block_grad.grad_time_state.clone(),time_prev:block_grad.grad_time_prev.clone(),cmix_prev:block_grad.grad_cmix_prev.clone(),v_first:Some(block_grad.grad_v_first.clone())});
