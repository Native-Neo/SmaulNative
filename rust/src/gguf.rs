...
impl GgufReader {
    ...
    pub fn tensor_bytes(&mut self,name:&str)->Result<Vec<u8>,String>{let info=self.tensor(name).cloned().ok_or_else(||format!("GGUF tensor '{name}' not found"))?;if info.dtype!=0&&info.dtype!=1{return Err(format!("tensor '{name}' uses unsupported GGML type {} (reader currently supports F32/F16)",info.dtype));}let elements=info.shape.iter().try_fold(1u64,|a,&b|a.checked_mul(b)).ok_or("tensor element count overflow")?;let bytes_per=if info.dtype==0{4}else{2};let size=elements.checked_mul(bytes_per).ok_or("tensor byte size overflow")?;let size=usize::try_from(size).map_err(|_|"tensor is too large for memory")?;self.file.seek(SeekFrom::Start(self.data_start.checked_add(info.offset).ok_or("tensor offset overflow")?)).map_err(|e|e.to_string())?;let mut bytes=vec![0u8;size];self.file.read_exact(&mut bytes).map_err(|e|e.to_string())?;Ok(bytes)}
    pub fn tensor_f32(&mut self,name:&str)->Result<Vec<f32>,String>{let info=self.tensor(name).cloned().ok_or_else(||format!("GGUF tensor '{name}' not found"))?;let bytes=self.tensor_bytes(name)?;if info.dtype==0{return Ok(bytes.chunks_exact(4).map(|b|f32::from_le_bytes(b.try_into().unwrap())).collect());}Ok(bytes.chunks_exact(2).map(|b|f16_to_f32(u16::from_le_bytes([b[0],b[1]]))).collect())}
}
