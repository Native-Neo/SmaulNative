...
fn block_size(dtype:u32)->Option<(u64,u64)>{match dtype{2=>Some((32,18)),3=>Some((32,20)),6=>Some((32,22)),7=>Some((32,24)),8=>Some((32,34)),9=>Some((32,36)),_=>None}}
...
impl GgufReader {
    ...
    fn tensor_size(info:&GgufTensorInfo)->Result<usize,String>{
        ...
        let bytes=match info.dtype{0=>elements.checked_mul(4),1|30=>elements.checked_mul(2),24=>Some(elements),25=>elements.checked_mul(2),26=>elements.checked_mul(4),27|28=>elements.checked_mul(8),2|3|6|7|8|9=>{let(block,size)=block_size(info.dtype).unwrap();if elements%block!=0{return Err(format!("tensor '{}' has {} elements, not divisible by quant block size {}",info.name,elements,block));}elements.checked_div(block).and_then(|n|n.checked_mul(size))},_=>None}.ok_or_else(||format!("unsupported GGML type {} ({})",info.dtype,gguf_dtype_name(info.dtype)))?;
        usize::try_from(bytes).map_err(|_|"tensor is too large for memory".into())
    }
    pub fn tensor_bytes(&mut self,name:&str)->Result<Vec<u8>,String>{
        let info=self.tensor(name).cloned().ok_or_else(||format!("GGUF tensor '{name}' not found"))?;
        let size=Self::tensor_size(&info)?;let start=self.data_start.checked_add(info.offset).ok_or("tensor offset overflow")?;
        let end=start.checked_add(u64::try_from(size).map_err(|_|"tensor size overflow")?).ok_or("tensor end overflow")?;
        let file_len=self.file.metadata().map_err(|e|e.to_string())?.len();if end>file_len{return Err(format!("tensor '{}' extends past end of GGUF file",info.name));}
        self.file.seek(SeekFrom::Start(start)).map_err(|e|e.to_string())?;let mut bytes=vec![0u8;size];self.file.read_exact(&mut bytes).map_err(|e|e.to_string())?;Ok(bytes)
    }
    pub fn tensor_f32(&mut self,name:&str)->Result<Vec<f32>,String>{let info=self.tensor(name).cloned().ok_or_else(||format!("GGUF tensor '{name}' not found"))?;let bytes=self.tensor_bytes(name)?;match info.dtype{0=>Ok(bytes.chunks_exact(4).map(|b|f32::from_le_bytes(b.try_into().unwrap())).collect()),1=>Ok(bytes.chunks_exact(2).map(|b|f16_to_f32(u16::from_le_bytes([b[0],b[1]]))).collect()),30=>Ok(bytes.chunks_exact(2).map(|b|f32::from_bits(u32::from(u16::from_le_bytes([b[0],b[1]]))<<16)).collect()),24=>Ok(bytes.into_iter().map(|x|x as i8 as f32).collect()),25=>Ok(bytes.chunks_exact(2).map(|b|i16::from_le_bytes([b[0],b[1]]) as f32).collect()),26=>Ok(bytes.chunks_exact(4).map(|b|i32::from_le_bytes(b.try_into().unwrap()) as f32).collect()),27=>Ok(bytes.chunks_exact(8).map(|b|i64::from_le_bytes(b.try_into().unwrap()) as f32).collect()),28=>Ok(bytes.chunks_exact(8).map(|b|f64::from_le_bytes(b.try_into().unwrap()) as f32).collect()),2=>Ok(decode_q4_0(&bytes)),3=>Ok(decode_q4_1(&bytes)),6=>Ok(decode_q5_0(&bytes)),7=>Ok(decode_q5_1(&bytes)),8=>Ok(decode_q8_0(&bytes)),9=>Ok(decode_q8_1(&bytes)),_=>Err(format!("tensor '{}' uses unsupported GGML type {} ({})",info.name,info.dtype,gguf_dtype_name(info.dtype)))}}
}
...
fn decode_q8_1(bytes:&[u8])->Vec<f32>{let mut out=Vec::with_capacity(bytes.len()/36*32);for block in bytes.chunks_exact(36){let d=read_f32(&block[..4]);let m=read_f32(&block[4..8]);for i in 0..32{out.push(d*(block[8+i] as i8 as f32)+m);}}out}
fn read_f32(b:&[u8])->f32{f32::from_le_bytes(b[..4].try_into().unwrap())}
