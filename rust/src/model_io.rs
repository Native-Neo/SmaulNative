...
impl PretrainedModel {
    pub fn load(directory: impl AsRef<Path>) -> Result<Self, String> {
        ...
        load_model_safetensors(&mut model, directory.join("model.safetensors"))?;
        Ok(Self { config, model, tokenizer })
    }

    pub fn save_model(&self, directory: impl AsRef<Path>) -> Result<(), String> {
        let directory = directory.as_ref();
    ...
    pub fn paths(directory: impl AsRef<Path>) -> (PathBuf, PathBuf, PathBuf) {
        let directory = directory.as_ref();
        (directory.join("config.json"), directory.join("model.safetensors"), directory.join("tokenizer.json"))
    }
}
...
mod tests {
    ...
    #[test]
    fn standard_paths_are_stable() {
        let (config, model, tokenizer) = PretrainedModel::paths("model");
        assert_eq!(config, PathBuf::from("model/config.json"));
        assert_eq!(model, PathBuf::from("model/model.safetensors"));
        assert_eq!(tokenizer, PathBuf::from("model/tokenizer.json"));
    }
}
