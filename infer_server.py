...
<style>
:root{--bg:#101010;--panel:#171717;--panel2:#202020;--text:#f1f1f1;--muted:#9b9b9b;--line:#2d2d2d;--accent:#d6b4ff;--user:#262626;--danger:#ff7f7f}
...
<script>
let messages=[], chats=JSON.parse(localStorage.getItem('smaul-chats')||'[]'), busy=false;
const $=id=>document.getElementById(id);const input=$('input');
function esc(s){return s.replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}
function render(s){let x=esc(s);x=x.replace(/```([\s\S]*?)```/g,'<pre style="background:#0b0b0b;padding:14px;border-radius:10px;overflow:auto"><code>$1</code></pre>');x=x.replace(/`([^`]+)`/g,'<code style="background:#252525;padding:2px 5px;border-radius:5px">$1</code>');x=x.replace(/\*\*([^*]+)\*\*/g,'<strong>$1</strong>');x=x.replace(/\n/g,'<br>');return x}
function draw(){let m=$('messages');if(!messages.length){m.innerHTML='<div class="welcome"><h1>How can I help?</h1><p>Your conversation stays on this machine.</p></div>';return}m.innerHTML=messages.map((x,i)=>`<div class="msg ${x.role}"><div class="role">${x.role==='user'?'You':'SmaulNative'}</div><div class="bubble">${render(x.content)}</div></div>`).join('');$('chat').scrollTop=$('chat').scrollHeight}
function save(){localStorage.setItem('smaul-chats',JSON.stringify(chats.slice(-30)));localStorage.setItem('smaul-current',JSON.stringify(messages))}
function history(){let h=$('history');h.innerHTML=chats.map((c,i)=>`<button onclick="loadChat(${i})">${esc(c.title||'New chat')}</button>`).join('')}
function loadChat(i){messages=chats[i].messages||[];draw();save()}
function newChat(){messages=[];draw();save();input.focus()}
async function send(){if(busy)return;let text=input.value.trim();if(!text)return;input.value='';input.style.height='48px';messages.push({role:'user',content:text});messages.push({role:'assistant',content:''});draw();busy=true;$('send').disabled=true;$('status').textContent='Thinking…';let started=performance.now();try{let r=await fetch('/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'rwkv-x',messages:messages.slice(0,-1),stream:true,max_tokens:256,temperature:.7,top_p:.95})});if(!r.ok)throw new Error(await r.text());let reader=r.body.getReader(),dec=new TextDecoder(),buf='';while(true){let q=await reader.read();if(q.done)break;buf+=dec.decode(q.value,{stream:true});let parts=buf.split('\n');buf=parts.pop();for(let line of parts){if(!line.startsWith('data: '))continue;let d=line.slice(6);if(d==='[DONE]')continue;try{let j=JSON.parse(d);let c=j.choices?.[0]?.delta?.content||'';messages[messages.length-1].content+=c;draw()}catch(e){}}}let sec=(performance.now()-started)/1000; $('status').textContent='Ready · '+((messages[messages.length-1].content.length/4)/Math.max(sec,.01)).toFixed(1)+' tok/s';let title=messages.find(x=>x.role==='user')?.content?.slice(0,42)||'New chat';chats.push({title,messages:[...messages]});save();history()}catch(e){messages[messages.length-1].content='Error: '+e.message;draw();$('status').textContent='Error'}finally{busy=false;$('send').disabled=false}}
input.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}});input.addEventListener('input',()=>{input.style.height='auto';input.style.height=Math.min(input.scrollHeight,180)+'px'});
try{let old=JSON.parse(localStorage.getItem('smaul-current'));if(Array.isArray(old))messages=old}catch(e){};history();draw();fetch('/health').then(r=>r.json()).then(x=>{$('device').textContent=x.device;$('modelmeta').textContent=x.parameters+' params'}).catch(()=>{});
</script></body></html>'''
...
def create_app(engine: RWKVXInference):
    ...
    async def chat(req: ChatRequest):
        ...
        system = req.system
        if system is None:
            system = "You are SmaulNative, a helpful local AI assistant. Be concise, accurate, and practical."
        ...
        text = "".join(chunks()).split('"content": ')[-1]
