...
#[cfg(test)]mod tests{use super::*;#[test]fn central_difference_is_correct_for_square(){let mut x=3.0;let grad=finite_difference(&mut x,1e-4,|v|v*v);assert!((grad-6.0).abs()<1e-3);}#[test]fn validates_wkv_state_shape(){let state=Array4::zeros((1,2,4,4));state_shape(&state,2,4);}}
