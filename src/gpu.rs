...
pub fn fp8_e4m3(code:u8)->f32{if code==0x7f||code==0xff{return f32::NAN}if code&0x7f==0{return if code&0x80!=0{-0.0}else{0.0}}let sign=if code&0x80!=0{-1.0}else{1.0};let exponent=((code>>3)&0x0f)as i32-7;let mantissa=(code&7)as f32;sign*2.0_f32.powi(exponent)*(1.0+mantissa/8.0)}
...
#[test]fn fp8_packed_is_direct(){let y=packed_linear_fp8_cpu(&[1.0,2.0],&[0,0],1,1,2).unwrap();assert_eq!(y,vec![0.0]);assert!(fp8_e4m3(0x7f).is_nan());assert!(fp8_e4m3(0xff).is_nan())}
