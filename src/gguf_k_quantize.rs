...
fn quantize_q3_k(values: &[f32]) -> Result<Vec<u8>, String> {
    if values.len() % 256 != 0 {
        return Err("Q3_K requires tensor size divisible by 256".into());
    }
    ...
    for block in values.chunks_exact(256) {
        ...
        if d != 0.0 {
            for group in 0..16 {
                ...
                for i in 0..16 {
                    let value = block[group * 16 + i];
                    q[group * 16 + i] = (round_i32(value / scale).clamp(-4, 3) + 4) as u8;
                }
            }
        }
...
fn quantize_q6_k(values: &[f32]) -> Result<Vec<u8>, String> {
    if values.len() % 256 != 0 {
        return Err("Q6_K requires tensor size divisible by 256".into());
    }
    ...
    for (key, value) in &reader.metadata {
        match value {
            ...
            GgufValue::Array(values)
                if values.iter().all(|v| matches!(v, GgufValue::String(_))) =>
            {
                let tokens = values
                    .iter()
                    .map(|v| match v {
...
pub fn quantize(input: impl AsRef<Path>, output: impl AsRef<Path>, kind: &str) -> Result<(), String> {
    if !matches!(kind, "q3_k" | "q6_k") {
        return Err("K quantization must be q3_k or q6_k".into());
    }
    ...
    for info in tensors {
        ...
        let (dtype, data) = if values.len() % 256 == 0 {
            ...
            let dtype = if kind == "q3_k" { 11 } else { 14 };
            (dtype, data)
        } else {
