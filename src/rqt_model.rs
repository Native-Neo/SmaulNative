...
impl RqtModel {
    ...
    pub fn refresh_quantized(&mut self) -> Result<(),String> {
        ...
        for b in &mut self.model.rwkv_blocks {
            q!(&mut b.time_mix.w1); q!(&mut b.time_mix.w2); q!(&mut b.time_mix.a1); q!(&mut b.time_mix.a2);
            if let Some(v)=&mut b.time_mix.v1 { q!(v); } if let Some(v)=&mut b.time_mix.v2 { q!(v); }
            q!(&mut b.time_mix.g1); q!(&mut b.time_mix.g2); q!(&mut b.time_mix.receptance); q!(&mut b.time_mix.key); q!(&mut b.time_mix.value); q!(&mut b.time_mix.output);
            q!(&mut b.cmix.key); q!(&mut b.cmix.value);
            if let Some(m)=&mut b.moe { q!(&mut m.router.weight); for e in &mut m.experts { q!(&mut e.key); q!(&mut e.value); } }
        }
        for b in &mut self.model.moba_blocks {
            ...
            q!(&mut b.ffn.key); q!(&mut b.ffn.value);
        }
        Ok(())
    }
