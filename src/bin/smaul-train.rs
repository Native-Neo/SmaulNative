...
enum Data {
    Single(DatasetStream),
    Multi(MultiFileDatasetStream),
    Sft(SftDataset),
}

impl Data {
    fn next(&mut self) -> Result<Option<TokenBatch>, String> {
        match self {
            Self::Single(x) => x.next_batch(),
            Self::Multi(x) => x.next_batch(),
            Self::Sft(_) => Ok(None),
        }
    }
}
...
fn cosine_lr(base: f32, min: f32, step: usize, total: usize, warmup: usize) -> f32 {
    ...
    let p = if total <= warmup {
        1.0
    } else {
        (step.saturating_sub(warmup) as f32 / (total - warmup) as f32).min(1.0)
    };

    min + (base - min) * 0.5 * (1.0 + (std::f32::consts::PI * p).cos())
}
...
fn sft_step(
    ...
    for r in 0..logits.nrows() {
        ...
        let logz = m + sum.ln();
        loss += logz - row[target as usize];

        for c in 0..logits.ncols() {
            grad[[r, c]] = (row[c] - m).exp() / sum;
...
fn run() -> Result<(), String> {
    ...
    let model_path = if args.resume && args.output_dir.join("config.json").exists() {
        args.output_dir.clone()
    } else {
        args.model_dir.clone()
    };

    require_directory(&model_path, "model")?;
    require_file(&model_path.join("config.json"), "model config")?;

    if args.mode == "sft" {
        require_file(&args.dataset, "SFT dataset")?;
    } else if args.dataset.is_dir() {
        require_directory(&args.dataset, "dataset")?;
    } else {
        require_file(&args.dataset, "dataset")?;
    }

    let mut pretrained = PretrainedModel::load(&model_path).map_err(|e| {
        format!(
    ...
    let mut data = if args.mode == "sft" {
        Data::Sft(SftDataset::open(
            &args.dataset,
            pretrained.tokenizer.clone(),
            args.ctx_len,
        )?)
    } else if args.dataset.is_dir() {
        Data::Multi(MultiFileDatasetStream::open_discovered(
            &args.dataset,
            pretrained.tokenizer.clone(),
            args.ctx_len,
        )?)
    } else {
        Data::Single(DatasetStream::open(
            &args.dataset,
            pretrained.tokenizer.clone(),
            args.ctx_len,
        )?)
    ...
    let kind = if args.optimizer == "adamw" {
        OptimizerKind::AdamW
    } else {
        OptimizerKind::Lion
    };

    let mut train = TrainStep::new_with_optimizer(&pretrained.model, args.lr, kind);
    let opt_path = args.output_dir.join("optimizer.bin");

    if args.resume && opt_path.exists() {
        let bytes = fs::read(&opt_path).map_err(|e| {
            format!("failed to read optimizer state {}: {e}", opt_path.display())
        })?;
        train.load_optimizer_state_bytes(&bytes)?;
    }

    println!("[MODEL] {} parameters", pretrained.model.parameter_count());
    println!(
            ...
            for _ in 0..args.batch_size {
                ...
                let step = if args.mode == "sft" {
                    ...
                    let batch = match data.next()? {
                        ...
                        None => {
                            return Err(
                                "dataset ended before training reached requested steps".into()
                            )
                        }
                    };
                ...
                if let Some(g) = &mut accumulated {
                    ...
                    accumulated = Some(step.gradients);
                }
            }
        }

        let gradients = accumulated.ok_or("no gradients accumulated")?;
        train.step(&mut pretrained.model, &gradients);
        state.step += 1;

        if state.step % args.log_every == 0 {
            println!(
                "step {} | loss {:.5} | lr {:.3e} | tokens {}",
                state.step,
                loss_sum / samples.max(1) as f32,
                lr,
                state.tokens
            );
        }

        if state.step % args.save_every == 0 || state.step == args.steps {
            ...
            pretrained.save(&args.output_dir)?;
            fs::write(&opt_path, train.optimizer_state_bytes()).map_err(|e| {
                format!("failed to save optimizer state {}: {e}", opt_path.display())
            })?;
