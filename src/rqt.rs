...
impl RealQuantLinear {
 ...
 pub fn forward(&self,input:&Array2<f32>)->Array2<f32>{let values=crate::gpu::packed_linear(input.as_slice().unwrap(),&self.quant.packed,&self.quant.scales,input.nrows(),self.quant.shape.0,self.quant.shape.1,self.bits.bits()).unwrap();let mut out=Array2::from_shape_vec((input.nrows(),self.quant.shape.0),values).unwrap();if let Some(b)=&self.bias{assert_eq!(b.len(),out.ncols());for r in 0..out.nrows(){for c in 0..out.ncols(){out[[r,c]]+=b[c];}}}out}
...
#[cfg(test)]
mod tests{use super::*;use ndarray::array;#[test]fn packed_training_roundtrip(){let l=RealQuantLinear::new(array![[1.0,0.0],[0.0,1.0]],None,4).unwrap();let y=l.forward(&array![[2.0,3.0]]);assert_eq!(y.shape(),&[1,2]);assert_eq!(l.dequantized().shape(),&[2,2]);}#[test]fn supported_bits(){for b in [2,4,8]{assert!(RqtConfig::new(b).is_ok())}}#[test]fn direct_fp8_forward(){let l=RealQuantLinear::new(array![[1.0,0.0]],None,8).unwrap();let y=l.forward(&array![[2.0,3.0]]);assert_eq!(y.shape(),&[1,1]);assert!(y[[0,0]].is_finite());}}
