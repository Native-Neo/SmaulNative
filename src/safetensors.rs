...
impl SafetensorsLoader {
    ...
    pub fn metadata(&self, key: &str) -> Result<String, String> { self.tensors()?.metadata().map_err(|e| format!("invalid safetensors metadata: {e}"))?.metadata().get(key).cloned().ok_or_else(|| format!("missing safetensors metadata '{key}'")) }
